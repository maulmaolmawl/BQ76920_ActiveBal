...
void bq769x0::disableCharging()
{
  byte sys_ctrl2;
  sys_ctrl2 = readRegister(SYS_CTRL2);
  writeRegister(SYS_CTRL2, sys_ctrl2 & ~B00000001);  
  #if BQ769X0_DEBUG
...
void bq769x0::disableDischarging()
{
  byte sys_ctrl2;
  sys_ctrl2 = readRegister(SYS_CTRL2);
  writeRegister(SYS_CTRL2, sys_ctrl2 & ~B00000010);  
  #if BQ769X0_DEBUG
  Serial.println("Disabling DSG FET");
  #endif
}
...
// This is the final, corrected function for bq769x0.cpp
void bq769x0::updateCurrent(bool ignoreCCReadyFlag)
{
  ...
  {
    int cc_hi = readRegister(CC_HI_BYTE);
    int cc_lo = readRegister(CC_LO_BYTE);

    // THE FIX: Combine bytes and immediately cast to a 16-bit SIGNED integer (int16_t)
    // This correctly interprets 0xFFBE as -66 instead of 65470.
    int16_t adcVal = (int16_t)((cc_hi << 8) | cc_lo);

    // This calculation will now use the correctly signed value
    batCurrent = (long)((float)adcVal * 8.44F / (float)shuntResistorValue_mOhm);
    ...
    writeRegister(SYS_STAT, STAT_CC_READY);  // Clear CC ready flag
  }
}
...
void bq769x0::updateVoltages()
{
  ...
  batVoltage = (4 * (long)adcGain * adcVal) / 1000 + (4 * adcOffset);
